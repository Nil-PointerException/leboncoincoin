...
class ConversationResourceTest {
    ...
    @Test
    @Order(8)
    @DisplayName("Should retrieve all messages in a conversation")
    void testGetMessagesInConversation() {
        ...
        given()
            ...
            .then()
                .statusCode(200)
                .contentType(ContentType.JSON)
                .body("$", hasSize(2)) // Two messages sent
                .body("[0].content", equalTo("Bonjour, est-ce que l'iPhone est toujours disponible ?"))
                .body("[1].content", equalTo("Je peux venir le chercher ce week-end si ça vous convient."))
                .body("[0].isRead", equalTo(false))
                .body("[1].isRead", equalTo(false));

        System.out.println("✅ Retrieved all messages in conversation");
    }
    ...
    void testMarkMessageAsRead() {
        ...
        given()
            .when()
                .put("/api/messages/" + firstMessageId + "/read")
            .then()
                .statusCode(200)
                .contentType(ContentType.JSON)
    ...
    void testVerifyMessageIsRead() {
        ...
        given()
            ...
            .then()
                .statusCode(200)
                .contentType(ContentType.JSON)
                .body("[0].isRead", equalTo(true))  // First message is read
                .body("[1].isRead", equalTo(false)); // Second message is still unread
    ...
    void testMarkNonExistentMessageAsRead() {
        given()
            .when()
                .put("/api/messages/non-existent-id/read")
            .then()
                .statusCode(404);
